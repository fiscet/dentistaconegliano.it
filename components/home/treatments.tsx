...
export type TreatmentsHeader = NonNullable<HOME_PAGE_QUERY_RESULT>["treatments"];
export type TreatmentsServices = HOME_SERVICES_QUERY_RESULT;
...
export default function Treatments({
  ...
  header?: TreatmentsHeader | null;
  services?: TreatmentsServices | null;
}) {
  // Fonte primaria: i servizi con "Mostra in home"; fallback ai contenuti
  // hardcoded quando non ci sono ancora servizi flaggati su Sanity.
  const items: Card[] = services?.length
    ? services.map((s) => ({
        key: s._id,
        title: s.title ?? "",
        description: s.description ?? "",
        href: s.slug ? `/servizi/${s.slug}` : "/servizi",
        icon: s.icon,
      }))
